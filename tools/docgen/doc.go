...
// 制御文字 (C0・C1・DEL)・双方向制御・U+2028/2029・不正な UTF-8・CR は、除去せず error にする。
...
//   - 書き込みは非原子的 (os.Root に Rename が無い)。途中で失敗すると壊れた出力が残るが、-check が検出する。
//   - os.Root は、bind mount・/proc・デバイスファイルを禁止しない。open の後の差し替えは、読む・切り詰める前の確認で見るが、
//     確認の後の差し替えと、root の外の実体は扱わない。
