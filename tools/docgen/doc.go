...
//   - 長い二項式の鎖 (var X = a+a+…+a の単一行) は、go/printer が二次時間になる (20 KB で 1.2 秒、40 KB で 5.1 秒、160 KB で
//     70 秒で強制終了)。... に省くのは複数行の値だけ。時間の増幅だけで、fail-closed (予算 60 秒 + 猶予 10 秒の watchdog)。
//     直すなら、長い値を、行数に関係なく ... に省く。時間の測定は不安定なので、固定するテストは置かない。
