...
- symlink のディレクトリは辿らない (go tool の `./...` と同じ)。一方、symlink の `.go` は go tool が通常のファイルとして build する (実測: `go list` の `GoFiles` に入る) ため、link の位置のファイルとして検査する。追わないと、許可された場所 (`sandbox/**`) のファイルへの symlink を `core/` に置くだけで規則をすり抜けられる。symlink の `go.mod` も go tool はそのまま読む (実測: 指す先の `module` が workspace の module path になる) ため、同じく link の位置で検査する (`modpath`)。
...
規則の一覧 (`exec-import` / `exec-call` / `cgo` / `dep-core` / `dep-agent-sandbox` / `impl-only-from-cmd` / `modpath`) は `tools/archtest/rules.go` を正とする。
...
- **既知の限界 (走査しないディレクトリ)**: `testdata` / `vendor` / `.` または `_` で始まるディレクトリと、symlink のディレクトリは走査しない (go tool の `./...` の列挙と同じ)。しかし go tool は、そこにある package も、明示的に import されれば build する。実測: `core/_hidden`、`core/.hidden`、`core/testdata`、`core/vendor`、`sandbox/` 配下へのディレクトリ symlink (`core/runner`) のいずれも、`core` から import すると `go build` が通って `os/exec` に依存し、archtest は緑のままだった。意図して置かれた迂回は検出できないため、レビューで見る。閉じるなら、in-repo の import path に走査しないディレクトリの名前が含まれたら違反にする、ディレクトリの symlink を error にする (または辿る)、の 2 つが要る。
