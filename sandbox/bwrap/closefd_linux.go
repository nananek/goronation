...
// Go の os/exec は、Stdin・Stdout・Stderr・ExtraFiles 以外を閉じず、bwrap も閉じないので、呼び手が継承した
// (CLOEXEC でない) fd は、そのまま檻のコマンドに届く。それを止める。呼び手のプロセス全体に効く。
// まず close_range を使い、どんな理由で失敗しても (5.9 未満は ENOSYS、5.9〜5.10 は flag が無く EINVAL、
// seccomp は EPERM)、/proc/self/fd を辿る。
